...
public class ConnectToDB {
    ...
    static boolean createAdmin(Statement statement, ResultSet resultset) throws Exception {
        ...
        String request = "INSERT INTO ACUSER(USERNAME,PASSWORD,FIRSTNAME,LASTNAME,EMAIL,ISCANDIDATE) VALUES ('Admin','"
                + Protector.encrypt("changeit") + "','Admin','Admin','gsn.administrator@host.com','no')";
    ...
    /*
     * create a Vector of names of all AC DB tables, the order of elements in Vector
     * is important because of the dependencies between tables
     */
    ...
    /*
     * return the index of missing AC table in the AC DB, and -1 if no table is
     * missing
     */
    ...
    /*
     * remove all AC DB Tables if one of them is missing, in fact if one AC table is
     * missing in the AC DB , it indicates
    ...
    /*
     * create all AC tables, order of creation is important because of the
    ...
    ResultSet selectOneColumnUnderOneCondition(Column col, String tableName, Column cond) throws SQLException {
        String query = "SELECT " + col.columnLabel + " FROM " + tableName + " WHERE " + cond.columnLabel + " = '"
                + cond.columnValue + "' ";
        return stmt.executeQuery(query);
    }

    // OK
    ResultSet selectOneColumnUnderTwoConditions(Column col, String tableName, Column firstCond, Column secondCond)
            throws SQLException {
        String query = "SELECT " + col.columnLabel + " FROM " + tableName + " WHERE " + firstCond.columnLabel + " = '"
                + firstCond.columnValue + "' AND " + secondCond.columnLabel + " = '" + secondCond.columnValue + "'";
        return stmt.executeQuery(query);
    }

    // OK
    ResultSet selectTwoColumnsUnderOneCondition(Column firstCol, Column secondCol, String tableName, Column cond)
            throws SQLException {

        String query = "SELECT " + firstCol.columnLabel + "," + secondCol.columnLabel + " FROM " + tableName + " WHERE "
                + cond.columnLabel + "='" + cond.columnValue + "'";
        return stmt.executeQuery(query);
    }

    ResultSet selectFiveColumns(Column firstCol, Column secondCol, Column thirdCol, Column fourthCol, Column fifthCol,
            String tableName) throws SQLException {

        String query = "SELECT " + firstCol.columnLabel + "," + secondCol.columnLabel + "," + thirdCol.columnLabel + ","
                + fourthCol.columnLabel + "," + fifthCol.columnLabel + " FROM " + tableName;
        return stmt.executeQuery(query);
    }

    // to be checked
    ResultSet selectTwoColumnsUnderTwoConditions(Column firstCol, Column secondCol, String tableName, Column firstCond,
            Column secondCond) throws SQLException {

        String query = "SELECT " + firstCol.columnLabel + "," + secondCol.columnLabel + " FROM " + tableName + " WHERE "
                + firstCond.columnLabel + " = '" + firstCond.columnValue + "' AND " + secondCond.columnLabel + " = '"
                + secondCond.columnValue + "'";
        return stmt.executeQuery(query);
    }

    // to be checked
    ResultSet selectTwoColumnsUnderThreeConditions(Column firstCol, Column secondCol, String tableName,
            Column firstCond, Column secondCond, Column thirdCond) throws SQLException {

        String query = "SELECT " + firstCol.columnLabel + "," + secondCol.columnLabel + " FROM " + tableName + " WHERE "
                + firstCond.columnLabel + " = '" + firstCond.columnValue + "' AND " + secondCond.columnLabel + " = '"
                + secondCond.columnValue + "' AND " + thirdCond.columnLabel + " = '" + thirdCond.columnValue + "'";
        return stmt.executeQuery(query);
    }

    ResultSet selectThreeColumnsUnderTwoConditions(Column firstCol, Column secondCol, Column thirdCol, String tableName,
            Column firstCond, Column secondCond) throws SQLException {

        String query = "SELECT " + firstCol.columnLabel + "," + secondCol.columnLabel + "," + thirdCol.columnLabel
                + " FROM " + tableName + " WHERE " + firstCond.columnLabel + " = '" + firstCond.columnValue + "' AND "
                + secondCond.columnLabel + " = '" + secondCond.columnValue + "'";
        return stmt.executeQuery(query);
    }
    ...
    // OK
    ResultSet selectAllColumnsUnderTwoConditions(String tableName, Column firstCond, Column secondCond)
            throws SQLException {
        String query = "SELECT * FROM " + tableName + " WHERE " + firstCond.columnLabel + " = '" + firstCond.columnValue
                + "' AND " + secondCond.columnLabel + " = '" + secondCond.columnValue + "'";
        return stmt.executeQuery(query);
    }
    ...
    // to be checked
    Vector getValuesVectorForOneColumnUnderTwoConditions(Column col, Column firstCond, Column secondCond,
            String tableName) throws SQLException {
        Vector values = new Vector();
    ...
    // OK
    String getValueForOneColumnUnderTwoConditions(Column col, Column firstCond, Column secondCond, String tableName)
            throws SQLException {
        String value = null;
    ...
    boolean insertTwoColumnsValues(Column firstCol, Column secondCol, String tableName) throws SQLException {
        boolean insertOK = false;
        String request = "INSERT INTO " + tableName + "(" + firstCol.columnLabel + "," + secondCol.columnLabel
                + ") VALUES ('" + firstCol.columnValue + "','" + secondCol.columnValue + "')";
    ...
    // OK
    boolean insertThreeColumnsValues(Column firstCol, Column secondCol, Column thirdCol, String tableName)
            throws SQLException {
        boolean insertOK = false;
        String request = "INSERT INTO " + tableName + "(" + firstCol.columnLabel + "," + secondCol.columnLabel + ","
                + thirdCol.columnLabel + ") VALUES ('" + firstCol.columnValue + "','" + secondCol.columnValue + "','"
                + thirdCol.columnValue + "')";
        int f = stmt.executeUpdate(request);
        if (f != 0) {
            insertOK = true;
        }
    ...
    boolean insertFourColumnsValues(Column firstCol, Column secondCol, Column thirdCol, Column fourthCol,
            String tableName) throws SQLException {
        boolean insertOK = false;
        String request = "INSERT INTO " + tableName + "(" + firstCol.columnLabel + "," + secondCol.columnLabel + ","
                + thirdCol.columnLabel + "," + fourthCol.columnLabel + ") VALUES ('" + firstCol.columnValue + "','"
                + secondCol.columnValue + "','" + thirdCol.columnValue + "','" + fourthCol.columnValue + "')";
        int f = stmt.executeUpdate(request);
        if (f != 0) {
            insertOK = true;
        }
    ...
    boolean insertSixColumnsValues(Column firstCol, Column secondCol, Column thirdCol, Column fourthCol,
            Column fifthCol, Column sixthCol, String tableName) throws SQLException {
        boolean insertOK = false;
        String request = "INSERT INTO " + tableName + "(" + firstCol.columnLabel + "," + secondCol.columnLabel + ","
                + thirdCol.columnLabel + "," + fourthCol.columnLabel + "," + fifthCol.columnLabel + ","
                + sixthCol.columnLabel + ") VALUES ('" + firstCol.columnValue + "','" + secondCol.columnValue + "','"
                + thirdCol.columnValue + "','" + fourthCol.columnValue + "','" + fifthCol.columnValue + "','"
                + sixthCol.columnValue + "')";
        int f = stmt.executeUpdate(request);
        if (f != 0) {
            insertOK = true;
        }
    ...
    boolean insertSeventhColumnsValues(Column firstCol, Column secondCol, Column thirdCol, Column fourthCol,
            Column fifthCol, Column sixthCol, Column seventhCol, String tableName) throws SQLException {
        boolean insertOK = false;
        String request = "INSERT INTO " + tableName + "(" + firstCol.columnLabel + "," + secondCol.columnLabel + ","
                + thirdCol.columnLabel + "," + fourthCol.columnLabel + "," + fifthCol.columnLabel + ","
                + sixthCol.columnLabel + "," + seventhCol.columnLabel + ") VALUES ('" + firstCol.columnValue + "','"
                + secondCol.columnValue + "','" + thirdCol.columnValue + "','" + fourthCol.columnValue + "','"
                + fifthCol.columnValue + "','" + sixthCol.columnValue + "','" + seventhCol.columnValue + "')";
        int f = stmt.executeUpdate(request);
        if (f != 0) {
            insertOK = true;
        }
    ...
    /*
     * This method will find all kind of condition value(for candidate registration,
     * group registration)
     */
    ...
    boolean valueExistsForThisColumnUnderTwoConditions(Column col, Column firstCond, Column secondCond,
            String tableName) throws SQLException {
        boolean valueFound = false;
        rs = selectOneColumnUnderTwoConditions(col, tableName, firstCond, secondCond);
    ...
    boolean isPasswordCorrectForThisUser(String userName, String password) throws Exception // password is already
                                                                                            // encrypted when arrives
                                                                                            // here!
    {
    ...
    User getOwnerOfDataSource(String dataSourceName) throws SQLException {
        return this.getUserForUserName(this.getValueForOneColumnUnderTwoConditions(new Column("USERNAME"),
                new Column("DATASOURCENAME", dataSourceName), new Column("DATASOURCETYPE", "4"),
                "ACUSER_ACDATASOURCE"));
    }
    ...
    Vector getDataSourceList() throws SQLException {
        return this.getValuesVectorForOneColumnUnderOneCondition(new Column("DATASOURCENAME"),
                new Column("ISCANDIDATE", "no"), "ACDATASOURCE");
    }

    /*
     * get list of all prospective users, everybody who has already signed up but
     * waits for Admin confirmation to become a user
     */
    Vector getUserCandidates() throws SQLException {
        String username = null;
        Vector usernamevector = this.getValuesVectorForOneColumnUnderOneCondition(new Column("USERNAME"),
                new Column("ISCANDIDATE", "yes"), "ACUSER");
        Vector uservector = new Vector();
        for (int i = 0; i < usernamevector.size(); i++) {
            username = (String) usernamevector.get(i);// WAITINGACUSER
            uservector.add(new User(username,
                    getValueForOneColumnUnderOneConditions(new Column("PASSWORD"), new Column("USERNAME", username),
                            "ACUSER"),
                    getValueForOneColumnUnderOneConditions(new Column("FIRSTNAME"), new Column("USERNAME", username),
                            "ACUSER"),
                    getValueForOneColumnUnderOneConditions(new Column("LASTNAME"), new Column("USERNAME", username),
                            "ACUSER"),
                    getValueForOneColumnUnderOneConditions(new Column("EMAIL"), new Column("USERNAME", username),
                            "ACUSER"),
                    getGroupListForUser(username), "yes"));
        }
        return uservector;
    }

    /*
     * get list of all users who are applied for an access right modification for
     * (a) group(s) or for (a) virtual sensor(s)
     */
    ...
    /*
     * get the list of groups(each element is a GROUP with groupName and GroupType,
     * the list of virtual sensors in group is not precised) for which a user is
     * waiting the access right modification
     */
    Vector getGroupsForWaitingUserFirstPart(String userName) throws SQLException {
        Vector groupVector = new Vector();
        rs = this.selectTwoColumnsUnderTwoConditions(new Column("GROUPNAME"), new Column("GROUPTYPE"), "ACUSER_ACGROUP",
                new Column("USERNAME", userName), new Column("ISUSERWAITING", "yes"));
        while (rs.next()) {
    ...
    /*
     * get the list of groups(each element is a GROUP with groupName and GroupType,
     * the list of virtual sensors in group is precised) for which a user is waiting
     * the access right modification
     */
    ...
    /*
     * get the list of virtual sensors existing in the group with their
     * coressponding access rights
     */
    Vector getDataSourcesForWaitingUser(String userName) throws SQLException {
        Vector dsVector = new Vector();

        rs = this.selectThreeColumnsUnderTwoConditions(new Column("DATASOURCENAME"), new Column("DATASOURCETYPE"),
                new Column("OWNERDECISION"), "ACUSER_ACDATASOURCE", new Column("USERNAME", userName),
                new Column("ISUSERWAITING", "yes"));
        while (rs.next()) {
            dsVector.add(new DataSource(rs.getString("DATASOURCENAME"), rs.getString("DATASOURCETYPE"),
                    rs.getString("OWNERDECISION")));
        }
        return dsVector;
    ...
    Vector getGroupListForUser(String userName) throws SQLException {
        ...
        Vector groupnamevector = this.getValuesVectorForOneColumnUnderOneCondition(new Column("GROUPNAME"),
                new Column("USERNAME", userName), "ACUSER_ACGROUP");
        Vector groupvector = new Vector();
        for (int i = 0; i < groupnamevector.size(); i++) {
            groupname = (String) groupnamevector.get(i);
            grouptype = this.getValueForOneColumnUnderTwoConditions(new Column("GROUPTYPE"),
                    new Column("GROUPNAME", groupname), new Column("USERNAME", userName), "ACUSER_ACGROUP");
    ...
    /*
     * given the groupname this method returns the list of all virtual sensors
     * access rights in the group
     */
    Vector getDataSourceListForGroup(String groupName) throws SQLException {
        Vector dsList = new Vector();
        rs = this.selectTwoColumnsUnderOneCondition(new Column("DATASOURCENAME"), new Column("DATASOURCETYPE"),
                "ACGROUP_ACDATASOURCE", new Column("GROUPNAME", groupName));
        // Display
        while (rs.next()) {
    ...
    /*
     * given the enterd parameters for a list virtual sensors, access rights from a
     * HTML form, retun the list of DataSource objects
     */
    Vector getDataSourceListForParameterSet(ParameterSet pm) throws SQLException {
        ...
        while (rs.next()) {
            if (pm.valueForName(rs.getString("DATASOURCENAME")) != null) {
                dsList.add(new DataSource(rs.getString("DATASOURCENAME"),
                        pm.valueForName(rs.getString("DATASOURCENAME"))));
            }
        }
        return dsList;
    }
    ...
    Vector getChangedDataSourceListForParameterSet(ParameterSet pm) throws SQLException {
        ...
        while (rs.next()) {
            if (pm.valueForName(rs.getString("DATASOURCENAME")) != null
                    && (pm.valueForName(rs.getString("DATASOURCENAME")).equals("0") == false)) {
                dsList.add(new DataSource(rs.getString("DATASOURCENAME"),
                        pm.valueForName(rs.getString("DATASOURCENAME"))));
            }
        }
    ...
    /*
     * given the username, return the data source list( virtual sensor, access
     * rights) to which user has access
     */
    Vector getDataSourceListForUser(String userName) throws SQLException {
        ...
        while (rs.next()) {
            dsList.add(new DataSource(rs.getString("DATASOURCENAME"), rs.getString("DATASOURCETYPE"),
                    rs.getString("FILENAME"), rs.getString("FILETYPE"), rs.getString("PATH"),
                    rs.getString("OWNERDECISION")));
        }
        return dsList;
    }
    ...
    /*
     * return the list of datasources(virtual sensor name, access rights) for an
     * owner
     */
    Vector getDataSourceNamesListForThisOwner(User owner) throws SQLException {
        DataSource ds = null;
        Vector dsNamesList = new Vector();
        // return getValuesVectorForOneColumnUnderTwoConditions(new
        // Column("DATASOURCENAME"),new Column("DATASOURCETYPE","4"),new
        // Column("USERNAME",ownername),"ACUSER_ACDATASOURCE");
    ...
    /*
     * return the list of users who have modified their access rights for this owner
     * virtual sensors and are waitin for his/her decision
     */
    Vector getUsersWaitingForThisOwnerDecision(String dsname) throws SQLException {
        Vector users = new Vector();
        String dataSourceType = null;
        String userName = null;
        rs = this.selectTwoColumnsUnderThreeConditions(new Column("USERNAME"), new Column("DATASOURCETYPE"),
                "ACUSER_ACDATASOURCE", new Column("DATASOURCENAME", dsname), new Column("OWNERDECISION", "notreceived"),
                new Column("ISUSERWAITING", "yes"));
        while (rs.next()) {
    ...
    boolean isDataSourceCandidate(DataSource ds) throws SQLException {
        ...
        if ((getValueForOneColumnUnderOneConditions(new Column("ISCANDIDATE"),
                new Column("DATASOURCENAME", ds.getDataSourceName()), "ACDATASOURCE")).equals("yes")) {
            isCandi = true;
        }
        return isCandi;
    ...
    /*
     * checks if user is waiting the owner decision for adding this virtual sensor,
     * charAt(0)=='5' indicates that user
    ...
    Vector getDataSourceCandidates() throws SQLException {
        String dsname = null;
        Vector dsnamevector = this.getValuesVectorForOneColumnUnderOneCondition(new Column("DATASOURCENAME"),
                new Column("ISCANDIDATE", "yes"), "ACDATASOURCE");
        Vector dsvector = new Vector();
    ...
    /*
     * given the name of a prospective virtual sensor, this method retrurns an
     * object DataSource with auxiliary information
     */
    DataSource getDataSourceCandidateForDataSourceName(String dataSourceName) throws SQLException {
        ...
        rs = this.selectAllColumnsUnderOneCondition("ACUSER_ACDATASOURCE",
    ...
    /*
     * given the name of the user and a virtual sensor, returns an object DataSource
     * which contains also access
    ...
    DataSource getDataSourceForUser(User user, String dataSourceName) throws SQLException {
        DataSource ds = null;
        rs = selectAllColumnsUnderTwoConditions("ACUSER_ACDATASOURCE", new Column("USERNAME", user.getUserName()),
                new Column("DATASOURCENAME", dataSourceName));
        while (rs.next()) {
    ...
    boolean registerGroup(Group group) throws Exception {
        ...
        if (this.insertOneColumnValue(new Column("GROUPNAME", group.getGroupName()), "ACGROUP") == true) {
            ...
            while (i < group.getDataSourceList().size() && (success == true)) {
                ...
                if (insertThreeColumnsValues(new Column("GROUPNAME", group.getGroupName()),
                        new Column("DATASOURCENAME", ds.getDataSourceName()),
                        new Column("DATASOURCETYPE", ds.getDataSourceType()), "ACGROUP_ACDATASOURCE") == false) {
                    success = false;
                }
                i++;
            }
    ...
    boolean registerDataSourceCandidate(DataSource ds) throws Exception {
        boolean success = false;
        if (this.insertTwoColumnsValues(new Column("DATASOURCENAME", ds.getDataSourceName()),
                new Column("ISCANDIDATE", ds.getIsCandidate()), "ACDATASOURCE") == true) {
    ...
    boolean registerUserCandidate(User user) throws SQLException {
        boolean success = false;
        Group group = null;
        if (insertSixColumnsValues(new Column("USERNAME", user.getUserName()),
                new Column("PASSWORD", user.getPassword()), new Column("FIRSTNAME", user.getFirstName()),
                new Column("LASTNAME", user.getLastName()), new Column("EMAIL", user.getEmail()),
                new Column("ISCANDIDATE", user.getIsCandidate()), "ACUSER") == true) {
            success = true;
            int i = 0;
    ...
    boolean registerGroupForUser(User user, Group group) throws SQLException {
        return this.insertFourColumnsValues(new Column("USERNAME", user.getUserName()),
                new Column("GROUPNAME", group.getGroupName()), new Column("GROUPTYPE", group.getGroupType()),
                new Column("ISUSERWAITING", user.getIsWaiting()), "ACUSER_ACGROUP");
    ...
    boolean registerDataSourceForUser(User user, DataSource ds) throws SQLException {
        return this.insertSeventhColumnsValues(new Column("USERNAME", user.getUserName()),
                new Column("DATASOURCENAME", ds.getDataSourceName()),
                new Column("DATASOURCETYPE", ds.getDataSourceType()), new Column("FILENAME", ds.getFileName()),
                new Column("FILETYPE", ds.getFileType()), new Column("PATH", ds.getPath()),
                new Column("ISUSERWAITING", user.getIsWaiting()), "ACUSER_ACDATASOURCE");
    ...
    boolean registerDataSourceForGroup(Group group, DataSource ds) throws SQLException {
        return this.insertThreeColumnsValues(new Column("GROUPNAME", group.getGroupName()),
                new Column("DATASOURCENAME", ds.getDataSourceName()),
                new Column("DATASOURCETYPE", ds.getDataSourceType()), "ACGROUP_ACDATASOURCE");
    ...
    boolean updateOneColumnUnderOneCondition(Column col, Column cond, String tableName) throws SQLException {
        String query = "UPDATE " + tableName + " SET " + col.columnLabel + "= '" + col.columnValue + "' WHERE "
                + cond.columnLabel + "= '" + cond.columnValue + "'";
    ...
    boolean updateOneColumnUnderTwoConditions(Column col, Column firstCond, Column secondCond, String tableName)
            throws SQLException {
        String query = "UPDATE " + tableName + " SET " + col.columnLabel + "= '" + col.columnValue + "' WHERE "
                + firstCond.columnLabel + "= '" + firstCond.columnValue + "' AND " + secondCond.columnLabel + "= '"
                + secondCond.columnValue + "'";
    ...
    public void updateDataSourceCandidate(String valueForName) throws SQLException {
        this.updateOneColumnUnderOneCondition(new Column("ISCANDIDATE", "no"),
                new Column("DATASOURCENAME", valueForName), "ACDATASOURCE");
    }
    ...
    void updateGroupForUser(User user, Group group) throws SQLException {
        this.updateOneColumnUnderTwoConditions(new Column("ISUSERWAITING", user.getIsWaiting()),
                new Column("USERNAME", user.getUserName()), new Column("GROUPNAME", group.getGroupName()),
                "ACUSER_ACGROUP");
        this.updateOneColumnUnderTwoConditions(new Column("GROUPTYPE", group.getGroupType()),
                new Column("USERNAME", user.getUserName()), new Column("GROUPNAME", group.getGroupName()),
                "ACUSER_ACGROUP");
    ...
    void updateDataSourceForUser(User user, DataSource ds) throws SQLException {
        this.updateOneColumnUnderTwoConditions(new Column("ISUSERWAITING", user.getIsWaiting()),
                new Column("USERNAME", user.getUserName()), new Column("DATASOURCENAME", ds.getDataSourceName()),
                "ACUSER_ACDATASOURCE");
        this.updateOneColumnUnderTwoConditions(new Column("DATASOURCETYPE", ds.getDataSourceType()),
                new Column("USERNAME", user.getUserName()), new Column("DATASOURCENAME", ds.getDataSourceName()),
                "ACUSER_ACDATASOURCE");
    ...
    void updateOwnerDecision(String decision, String userName, String dataSourceName) throws SQLException {
        this.updateOneColumnUnderTwoConditions(new Column("OWNERDECISION", decision),
                new Column("DATASOURCENAME", dataSourceName), new Column("USERNAME", userName), "ACUSER_ACDATASOURCE");
    }
    ...
    int deleteUnderTwoConditions(Column firstCond, Column secondCond, String tableName) throws SQLException {
        String query = "DELETE FROM " + tableName + " WHERE " + firstCond.columnLabel + "= '" + firstCond.columnValue
                + "' AND " + secondCond.columnLabel + "= '" + secondCond.columnValue + "'";
        return stmt.executeUpdate(query);
    ...
    void deleteGroupForUser(Group group, User user) throws SQLException {
        this.deleteUnderTwoConditions(new Column("GROUPNAME", group.getGroupName()),
                new Column("USERNAME", user.getUserName()), "ACUSER_ACGROUP");
    }

    void deleteDataSourceForUser(DataSource ds, User user) throws SQLException {
        this.deleteUnderTwoConditions(new Column("DATASOURCENAME", ds.getDataSourceName()),
                new Column("USERNAME", user.getUserName()), "ACUSER_ACDATASOURCE");
    }

    void deleteGroupListsDifferenceForUser(Vector oldGroupList, Vector newGroupList, String username)
            throws SQLException // for using this method oldVector.size()>= newVector.size()
    {
        Group gr = null;
        ...
        for (int i = 0; i < grdiffvector.size(); i++) {
            ...
            this.deleteUnderTwoConditions(new Column("USERNAME", username), new Column("GROUPNAME", gr.getGroupName()),
                    "ACUSER_ACGROUP");
        }
    }
    ...
    public Vector getGroupListsDifference(Vector oldVector, Vector newVector) throws SQLException // for using this
                                                                                                  // method
                                                                                                  // oldVector.size()>=
                                                                                                  // newVector.size()
    {
    ...
    public Vector getDataSourceListsDifference(Vector oldVector, Vector newVector) throws SQLException // for using this
                                                                                                       // method
                                                                                                       // oldVector.size()>=
                                                                                                       // newVector.size()
    {
