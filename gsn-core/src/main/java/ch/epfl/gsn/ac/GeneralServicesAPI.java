...
/*
 * This class helps to use access control general functionalities without using
...
public class GeneralServicesAPI {
    ...
    /*
     * this method return an object User if it succeeds to doLogin and null
     * otherwise
     */
    public User doLogin(String username, String password) {
        ...
        try {
            ...
            if (ctdb.valueExistsForThisColumnUnderOneCondition(new Column("USERNAME", username),
                    new Column("ISCANDIDATE", "no"), "ACUSER") == true) {
                ...
                if ((ctdb.isPasswordCorrectForThisUser(username, enc) == false)) {
                    ...
                    user = new User(username, enc, ctdb.getDataSourceListForUserLogin(username),
                            ctdb.getGroupListForUser(username));
                }
            } else {
                // throws exception
    ...
    /*
     * given the name of the group, this method returns the list of all DataSources
     * in the group
     */
