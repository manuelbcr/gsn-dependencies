...
public class RestStreamHandler extends HttpServlet implements ContinuationListener {
		...
		if (continuation.isInitial()) {
			...
			try {
				...
				if (Main.getContainerConfig().getAcConfig().isEnabled() && DataSource.isVSManaged(vsConfig.getName())) {
					...
					if (cred == null) {
						try {
							response.setHeader("WWW-Authenticate", "Basic realm=\"GSN Access Control\"");// set the
																											// supported
																											// challenge
						...
						if (rs.next()) {
							...
							if (rs.next()) {
								...
								if (startTime > t) {
									startTime = t;
									logger.info(
											"newest local timed is older than requested start time -> using timed as start time");
								}
							}
						} else {
							logger.info("Table '" + vsConfig.getName()
									+ "' doesn't exist => can not check for newest local timed");
						}
				...
				streamingReq = DefaultDistributionRequest.create(deliverySystem, parser.getVSensorConfig(),
						parser.getQuery(), startTime, parser.isContinuous());
			...
			if (((RestDelivery) streamingReq.getDeliverySystem()).isLimitReached()) {
				...
				DataDistributerRest.getInstance(streamingReq.getDeliverySystem().getClass())
						.removeListener(streamingReq);
			}
		}
	}
		...
		try {
			...
			if (Main.getContainerConfig().getAcConfig().isEnabled() && DataSource.isVSManaged(vsName)) {
				...
				if ((user == null || (!user.isAdmin() && !user.hasReadAccessRight(vsName)))) {
					response.setHeader("WWW-Authenticate", "Basic realm=\"GSN Access Control\"");// set the supported
																									// challenge
					response.sendError(HttpStatus.SC_UNAUTHORIZED, "Unauthorized access.");
					return;
				}
			...
			if (localContactPoint == null) {
				logger.warn(
						"Push streaming request received without " + PushDelivery.LOCAL_CONTACT_POINT + " parameter !");
				return;
			}
			// checking to see if there is an already registered notification id, in that
			// case, we ignore (re)registeration.
	...
	private String[] parseAuthorizationHeader(HttpServletRequest request) {
		...
		if (authHeader != null) {
			...
			if (ahs.length == 2) {
				...
				try {
					// String userPass = new String(decoder.decodeBuffer(b64UsernamPassword)); //
					// form: username:passsword
	...
	class URLParser {
		...
		public URLParser(HttpServletRequest request) throws UnsupportedEncodingException, Exception {
			...
			/**
			 * IMPORTANT: We change the table names to lower-case as some databases (e.g.,
			 * MySQL on linux) are case sensitive and in
