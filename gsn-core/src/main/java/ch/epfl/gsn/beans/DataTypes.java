...
public class DataTypes {
   ...
   // NEXT FIELD
   /**
    * Type Time is not supported at the moment. If you want to present time, please
    * use
    * longint. For more information consult the GSN mailing list on the same
    * subject.
    */
   ...
   public final static String[] TYPE_NAMES = new String[] { VARCHAR_NAME, CHAR_NAME, INTEGER_NAME, BIGINT_NAME,
         BINARY_NAME, DOUBLE_NAME,
         TIME_NAME,
         TINYINT_NAME,
         SMALLINT_NAME,
         FLOAT_NAME };
   ...
   public static byte convertTypeNameToGSNTypeID(final String type) {
      if (type == null) {
         throw new GSNRuntimeException(new StringBuilder("The type *null* is not recoginzed by GSN.")
               .append(DataTypes.ERROR_MESSAGE).toString());
      }
      ...
      throw new GSNRuntimeException(new StringBuilder("The type *").append(type).append("* is not recognized.")
            .append(DataTypes.ERROR_MESSAGE).toString());
   }
   ...
   public static byte SQLTypeToGSNTypeSimplified(int sqlType) {
      if (sqlType == Types.BIGINT || sqlType == Types.SMALLINT || sqlType == Types.DOUBLE || sqlType == Types.INTEGER
            || sqlType == Types.DECIMAL || sqlType == Types.REAL || sqlType == Types.FLOAT
            || sqlType == Types.NUMERIC) {
         return DataTypes.DOUBLE;
      } else if (sqlType == Types.VARCHAR || sqlType == Types.CHAR || sqlType == Types.LONGNVARCHAR
            || sqlType == Types.LONGVARCHAR || sqlType == Types.NCHAR) {
         return DataTypes.VARCHAR;
