...
public class DPPMessageMultiplexer implements BackLogMessageListener {
	...
	private Map<Integer, Vector<DPPMessagePlugin>> msgTypeListener = new HashMap<Integer, Vector<DPPMessagePlugin>>(); // Mapping
																														// from
																														// type
																														// to
																														// Listener
	...
	public synchronized static DPPMessageMultiplexer getInstance(String coreStationName,
			BackLogMessageMultiplexer blMsgMulti) throws Exception {
	...
	public boolean messageRecv(int deviceID, BackLogMessage message) {
		try {
			...
			while (en.hasMoreElements()) {
				...
				try {
					temp.activeBackLogWrapper.inputEvent(System.currentTimeMillis(),
							temp.activeBackLogWrapper.getRemoteConnectionPoint(), message.getSize());
				} catch (IOException e) {
			...
			if (ReceiverCount == 0) {
				logger.warn("Received message with type " + type
