 ...
 * </ul>
 * 
 * Each plugin type used by {@link gsn.wrappers.BackLogWrapper BackLogWrapper}
 * must
 ...
 * {@link gsn.wrappers.backlog.plugins.BackLogCommandPlugin
 * BackLogCommandPlugin}.</li>
...
public class BackLogMessage {
	 ...
	 * {@link gsn.wrappers.backlog.plugins.CoreStationStatusPlugin
	 ...
	 * 
	 * {@link gsn.wrappers.backlog.plugins.MotionDetectionPlugin
		...
		for (int i = 0; i < payload.length; i++) {
			if (payload[i] == null) {
				...
				throw new IOException(
						"unsupported type in payload (index=" + i + ", type=" + payload[i].getClass().getName() + ").");
			}
		}
