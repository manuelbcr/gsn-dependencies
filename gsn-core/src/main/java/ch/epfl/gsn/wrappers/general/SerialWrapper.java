 ...
 * through serial port.
 * <p/>
 * The only needed parameter is the serial port
...
public class SerialWrapper extends AbstractWrapper implements SerialPortEventListener {
	...
	public boolean initialize() {
		...
		if (representation == null || representation.equalsIgnoreCase("binary")) {
			...
			logger.error("The provided representation >" + representation
					+ "< is not valid, possible values are binary , string");
			return false;
		}
		...
		dataField = new DataField[] {
				new DataField(RAW_PACKET, (output_format == 0 ? "binary" : "varchar") + "(" + packet_length + ")",
						"The packet contains raw data from a sensor network.") };
		return true;
	}
	...
	public boolean sendToWrapper(Object dataItem) throws OperationNotSupportedException {
		if (logger.isDebugEnabled()) {
			logger.debug("Serial wrapper received a serial port sending...");
		}
		if (!wnetPort.isOpen()) {
		...
		try {
			if (logger.isDebugEnabled()) {
				logger.debug("Serial wrapper performing a serial port sending.");
			}
	...
	public void serialEvent(SerialPortEvent e) {
		...
		// serial port event, reading..." );
		// if ( !isActive( ) || listeners.isEmpty( ) ) {
		// if ( logger.isDebugEnabled( ) ) logger.debug( "Serial wrapper dropped the
		// input b/c there is no listener there or the wrapper is inactive." );
		// return;
		// }
		...
		if (logger.isDebugEnabled()) {
			logger.debug(new StringBuilder("Serial port wrapper processed a serial port event, stringbuffer is now : ")
					.append(new String(inputBuffer)).toString());
		}
	...
	private void post_item(String val) {
		switch (output_format) {
			case 0: // for binary data
				postStreamElement(
						val.length() > packet_length ? val.substring(0, packet_length).getBytes() : val.getBytes());
				break;
			case 1: // for strings
